#это файл с API endpoints для управления звонками через Asterisk ARI
...
from schemas.call_schemas import CallRequest, CallResponse, HangupRequest

# Инициализация роутера и логгера
router = APIRouter(prefix="/calls", tags=["calls"])
logger = get_logger(__name__)

# Зависимость для клиента ARI
def get_ari_client():
    return AriClient()
...
async def initiate_call(
    ...
    try:
        ...
        call_id = await ari_client.originate_call(
            ...
            extension=call_request.extension,
...
async def transfer_call(
    ...
    """
    Перевести звонок на другой extension
    """
    try:
...
async def play_audio_to_call(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=404, detail="Call not found or playback failed")
