...
def get_db_connection():
    ...
    # Регистрация адаптера и конвертера для UUID
...
def load_data_from_db():
    """Загружает данные из таблиц базы данных в глобальные pandas DataFrame."""
    global df_clients, df_regions, df_products
    try:
        conn = get_db_connection()
        df_clients = pd.read_sql_query("SELECT * FROM Clients_info", conn)
        df_regions = pd.read_sql_query("SELECT * FROM Regions_map", conn)
        ...
        if 'city_id' in df_regions.columns:
             df_regions['city_id'] = pd.to_numeric(df_regions['city_id'], errors='coerce')
...
class ClientProfile:
    ...
    def load_from_db(self, db_row):
        ...
        field_mapping = {
            ...
            "region_code": "client_region.code",
            "region_name": "client_region.name",
        }
...
def generate_json_data(profile, history, call_id, client_id):
    """Генерирует JSON на основе данных профиля и истории."""
...
def save_call_data_to_db(profile, history, client_id=None):
    ...
    try:
        ...
        if entered_phone and (client_id is None):
            ...
            sys_logger.debug(f"save_call_data_to_db: Предварительная проверка БД на существование клиента с номером '{normalized_search_phone}'")
        ...
        if is_new_client:
            ...
            profile_region_code = profile.get("client_region", {}).get("code") if isinstance(profile.get("client_region"), dict) else None
            ...
            client_data_to_insert = {
                ...
                'comment': final_comment if final_comment else None
            }
        ...
        last_call_summary = json_data.get("request_details", {}).get("request_text", "")
        ...
        if transcription:
            ...
            for i, replica in enumerate(transcription):
                transcript_data = {
                    ...
                    'segment_time': datetime.now().isoformat(),
        ...
        try:
            json_string_for_db = json.dumps(json_data, ensure_ascii=False)
            json_record = {
                'call_id': str(call_id),
                'client_id': str(client_id),
                'timestamp': datetime.now().isoformat(),
                'json_data': json_string_for_db
            }
            ...
            sys_logger.info("JSON данных звонка сохранен в таблицу jsons.")
             ...
             sys_logger.error(f"Ошибка при сохранении JSON в таблицу jsons: {e}")
             print(f"[Система] Предупреждение: Не удалось сохранить JSON в таблицу jsons: {e}")
