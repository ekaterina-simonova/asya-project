...
Этапы процесса
Этап Приветствие { Описание: приветствие клиента и запрос имени Профайлы: Общая информация Условия завершения этапа: имя клиента записано Агенты: -- Агент Приветствие  }
...
def get_agent_response(agent_name, profile=None, user_input=None):
    """Получаем ответ только через промпт без fallback-вопросов"""
    ...
    messages = [
        ...
        {"role": "user", "content": f"Агент {agent_name}. Текущие данные: {json.dumps(profile.data) if profile else 'нет данных'}"}
    ]
...
def save_to_excel(profile):
    ...
    profile.data["client_region"] = get_region(city)
...
def main():
    ...
    for agent in agents_sequence:
        while True:
            try:
                question = get_agent_response(agent, profile)
                ...
                # Сохраняем ввод в соответствующие поля
                if agent == "Агент Приветствие":
                    profile.update("Имя клиента", user_input)
                elif agent == "Агент Проверка номера":
                    profile.update("Телефон", user_input)
                elif agent == "Агент Запрос города":
                    profile.update("Город", user_input)
                elif agent == "Агент Проверка ИНН":
                    if user_input.isdigit():
                        profile.update("ИНН", user_input)
                    else:
                        profile.update("Организация", user_input)
                
                break  # Переход к следующему агенту
